...
const popupAddValidator = new FormValidator(validationElements, popupFormAdd);
...
const imagePopupPreview = new PopupWithImage(popupOpenImage);
...
const likeCard = (id, card) => {
  const likeRequest = card.getIsLiked()
    ? api.dislikeCard(id)
    : api.likeCard(id);
  likeRequest
    .then((res) => card._likeFunction(res))
    .catch((err) => console.log(err));
};
...
const createCard  = (data) =>{
  return new Card({
    ...
    template,
    handleClickCard: () => {
      imagePopupPreview.open(data.name, data.link);
    },
    handleRemoveCard: () => {
...
const addProfileInformation = () => {
  ...
  popupEditValidator.hideErrors();
  popupEditValidator.toggleButtonState();
};

const openAddCardPopup = () => {
  popupAddValidator.hideErrors();
